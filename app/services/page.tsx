...
export default function ServicesPage() {
    ...
    return (
        <div className="min-h-screen bg-black text-white px-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {mainServices.map((service, i) => (
                            <Link
                                key={service.title}
                                href={service.href}
                                className="group p-10 rounded-[40px] border border-zinc-900 hover:border-white/20 transition-all bg-black flex flex-col justify-between"
                            >
                                <div>
                                    <div className="w-12 h-12 rounded-2xl bg-zinc-900 flex items-center justify-center mb-8 border border-zinc-800 transition-colors group-hover:bg-white group-hover:text-black">
                                        <service.icon className="text-xl" />
                                    </div>
                                    <h2 className="text-3xl font-black italic tracking-tighter mb-4 uppercase group-hover:text-white transition-colors leading-none">{service.title}</h2>
                                    <p className="text-zinc-500 text-sm mb-8 leading-relaxed font-medium">{service.desc}</p>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-700">Explore Vertical</span>
            ...
            {/* Contact Section */}
            <section id="contact" className="py-40 border-t border-zinc-900 bg-black">
                <div className="max-w-7xl mx-auto">
